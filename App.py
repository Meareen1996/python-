...
class App:
    ...
    def CreateOptionMenu(self):
        ...
        pricingmenu.add_command(label = "KIKOPut", command = self.task8)
    ...
    def task8(self):
        ...
        # Labels to show params
        Label(frame, text = "KIKO Put Option Calculator", font = tkFont.Font(size = 11, weight = 'bold')).grid(row=1,column=1,sticky=W)
        Label(frame, text= 'Spot Price').grid(row=2,column=1,sticky=W)
        Label(frame, text= 'Strike Price').grid(row=3,column=1,sticky=W)
        Label(frame, text= 'Lower Barrier').grid(row=4,column=1,sticky=W)
        Label(frame, text= 'Upper Barrier').grid(row=5,column=1,sticky=W)
        Label(frame, text= 'Rebate').grid(row=6,column=1,sticky=W)
        Label(frame, text= 'Time to Maturity (in years)').grid(row=7,column=1,sticky=W)
        Label(frame, text= 'Number of Steps').grid(row=8,column=1,sticky=W)
        Label(frame, text= 'Volatility').grid(row=9,column=1,sticky=W)
        Label(frame, text= 'Risk Free Rate').grid(row=10,column=1,sticky=W)
        Label(frame, text= 'Number of Simulations').grid(row=11,column=1,sticky=W)
        ...
        Entry(frame, textvariable=self.S).grid(row=2,column=2,sticky=W)
        Entry(frame, textvariable=self.K).grid(row=3,column=2,sticky=W)
        Entry(frame, textvariable=self.L).grid(row=4,column=2,sticky=W)
        Entry(frame, textvariable=self.U).grid(row=5,column=2,sticky=W)
        Entry(frame, textvariable=self.R).grid(row=6,column=2,sticky=W)
        Entry(frame, textvariable=self.T).grid(row=7,column=2,sticky=W)
        Entry(frame, textvariable=self.n).grid(row=8,column=2,sticky=W)
        Entry(frame, textvariable=self.sigma).grid(row=9,column=2,sticky=W)
        ...
        # Reset input and log
        Button(frame, width = 20, text = "Reset Inputs",command=self.reset_task8).grid(row = 12, column = 1, columnspan = 1, sticky = E)

        # define run button to run the pricer
        Button(frame, width = 20, text = "Calculate",command=self.calculate_task8).grid(row = 12, column = 2, columnspan = 1, sticky = W)
    ...
    def calculate_task8(self):
        try:
            option = KIKOPut(float(self.S.get()), float(self.K.get()), float(self.L.get()), float(self.U.get()), float(self.R.get()), float(self.T.get()), int(self.n.get()), float(self.sigma.get()), float(self.r.get()), int(self.M.get()))
            result = option.payoff()
            self.logs.insert(END, "The Payoff is: {}\n".format(result))
        except:       
            self.logs.insert(END, "Input Parameter Error! Please input the correct parameters!\n")
